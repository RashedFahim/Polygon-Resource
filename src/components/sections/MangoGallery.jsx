      ...
      <div className="relative z-10 w-full">
        <div className="mb-8 px-3 sm:mb-10 sm:px-4 md:px-6 lg:px-8">
          <div className="mx-auto max-w-7xl text-center">
            ...
            <h2
              id="mango-gallery-title"
              className="font-['Lora',serif] text-[1.7rem] font-bold text-[#1F4732] sm:text-[2rem] lg:text-[2.3rem]"
            >
              <AnimatedText text="Mango Images" />
            </h2>
          </div>
        </div>

        <InfiniteMarquee edgeColor="#F7F4EA">
          {(copy) => MANGO_IMAGES.map((image, index) => (
            <div
              key={`${copy}-${image}`}
              className="group/mango mr-5 flex h-[220px] w-[250px] shrink-0 items-center justify-center overflow-hidden p-4 transition-transform duration-300 hover:-translate-y-1 sm:mr-7 sm:h-[260px] sm:w-[310px] sm:p-5 md:mr-8 md:h-[285px] md:w-[340px] lg:mr-10 lg:h-[310px] lg:w-[380px]"
            >
              <img
                src={image}
                alt={copy === 0 ? `Mango image ${index + 1}` : ''}
                className="h-full w-full object-contain transition-transform duration-300 group-hover/mango:scale-[1.03]"
                loading="lazy"
                draggable="false"
              />
            </div>
