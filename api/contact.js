...
const projectDirectory = path.join(apiDirectory, '..');

// Vercel provides process.env. The local files are only fallbacks for development.
dotenv.config({ path: path.join(projectDirectory, '.env.local') });
dotenv.config({ path: path.join(projectDirectory, '.env') });
dotenv.config({ path: path.join(projectDirectory, 'server', '.env') });

const logoPath = path.join(projectDirectory, 'public', 'logo.png');
const recipientEmail = readText(process.env.CONTACT_RECIPIENT_EMAIL)
  || readText(process.env.SMTP_USER);
...
const smtpUser = readText(process.env.SMTP_USER);
const smtpPass = readText(process.env.SMTP_PASS);
const smtpPort = Number.parseInt(process.env.SMTP_PORT || '465', 10);
const smtpSecure = process.env.SMTP_SECURE
  ? readText(process.env.SMTP_SECURE).toLowerCase() === 'true'
  : smtpPort === 465;
const transporter = smtpUser
  ...
  ? nodemailer.createTransport({
    host: readText(process.env.SMTP_HOST) || 'smtp.gmail.com',
    port: smtpPort,
    secure: smtpSecure,
    auth: {
      user: smtpUser,
...
export function getEmailStatus() {
  const missing = ['SMTP_USER', 'SMTP_PASS']
    .filter((name) => !readText(process.env[name]));
...
export async function handleContactRequest(req, res) {
  ...
  const { inquiry, errors } = validateInquiry(parseRequestBody(req.body));
  ...
  try {
    await transporter.sendMail({
      from: readText(process.env.SMTP_FROM) || smtpUser,
      to: recipientEmail,
